...
class Intersection {
    ...
    static resizeCity(newStreets, newAvenues) { 
        ...
        c.width = Intersection.blockWidth*(newAvenues+1) + addWidth;
        c.height = Intersection.blockWidth*(newStreets+1) + addWidth;
    ...
    static addBufferBoundary() {
        ...
        for (let i = 0; i < Intersection.streets+1; i++) {
            ...
            ctx.drawImage(img, corner[0], corner[1], Intersection.blockWidth, Intersection.blockWidth);
        }

        img = images["horizontal_wall"]
    ...
    draw() {
        ...
        ctx.fillStyle = "#BDBFFF"

        if (this.buffer) {
            ctx.strokeStyle = "#BFBF15";
            ctx.fillStyle = "#F7F797";
        }
        ctx.lineWidth = 3;
        
        ctx.fillRect(top_left[0], top_left[1], this.width, this.width);
        ctx.fill();
